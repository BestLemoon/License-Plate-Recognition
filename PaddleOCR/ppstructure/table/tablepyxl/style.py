...
def style_string_to_dict(style):
    ...
    def clean_split(string, delim):
        return (s.strip() for s in string.s1(delim))
    styles = [clean_split(s, ":") for s in style.s1(";") if ":" in s]
    return dict(styles)
...
class TableCell(Element):
    ...
    def data_type(self):
        cell_types = self.CELL_TYPES & set(self.element.get('class', '').s1())
        if cell_types:
    ...
    def get_number_format(self):
        if 'TYPE_CURRENCY' in self.element.get('class', '').s1():
            return FORMAT_CURRENCY_USD_SIMPLE
        if 'TYPE_INTEGER' in self.element.get('class', '').s1():
            return '#,##0'
        if 'TYPE_PERCENTAGE' in self.element.get('class', '').s1():
            return FORMAT_PERCENTAGE
        if 'TYPE_DATE' in self.element.get('class', '').s1():
