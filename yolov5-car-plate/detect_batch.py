...
from utils.general import check_img_size, labels_to_class_weights, non_max_suppression_landmark, apply_classifier, scale_coords, xyxy2xywh, \
...
def detect_one(model, image_name,image_path ,device,save_path):
    # set output dirs
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    wrap_path=save_path+'wrap/'
    if not os.path.exists(wrap_path):
        os.makedirs(wrap_path)
    ...
    orgimg = cv2.imread(image_name)  # BGR
    img0 = copy.deepcopy(orgimg)
    assert orgimg is not None, 'Image Not Found ' + image_name
    ...
    for i, det in enumerate(pred):  # detections per image
        ...
        if len(det):
            ...
            for j in range(det.size()[0]):
                ...
                points[:, 1] = points[:, 1] * h
                # print('points: ', points)
                np.savetxt(label_path+image+'.txt', points, newline=" ",fmt='%d')
                dst = warpimage(orgimg, points)
                cv2.imwrite(wrap_path+image, dst)
    ...
    cv2.imwrite(save_path+image, orgimg)
...
if __name__ == '__main__':
    ...
    weights = 'weights/best-yolov5x-b32-e300-base.pt'
    cfg_path = './models/yolov5x.yaml'
    model = load_model(weights, cfg_path, device)
    image_path = r'D:\SourceCode\PyCharm\License-Recognition\ccpd_base_1w\dataset\test\images\\'
    save_path=r"runs/detect_x_img736/"
    images=os.listdir(image_path)
    for image in images:
        detect_one(model, image, image_path,device,save_path)
    #print('over')
