...
def create_predictor(mode, logger):
    # if mode == "det":
    ...
    model_dir = "../reg/PaddleOCR/inference/"
...
#         opt_input_shape.update(opt_pact_shape)
    ...
    # if args.rec_algorithm not in ["CRNN", "SVTR_LCNet"]:
    #     use_dynamic_shape = False
    imgH = int("3, 32, 320".split(',')[-2])
    min_input_shape = {"x": [1, 3, imgH, 10]}
    max_input_shape = {"x": [6, 3, imgH, 2304]}
    opt_input_shape = {"x": [6, 3, imgH, 320]}
    config.exp_disable_tensorrt_ops(["transpose2"])
