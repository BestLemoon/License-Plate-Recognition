...
class DYShiftMax(nn.Layer):
    def __init__(self,
        ...
        indexgs = paddle.s1(index, [1, self.g - 1], axis=1)
        indexgs = paddle.concat((indexgs[1], indexgs[0]), axis=1)
        indexs = paddle.s1(indexgs, [1, self.gc - 1], axis=2)
    ...
    def forward(self, x):
        ...
        if self.exp == 4:
            temp = y.shape
            a1, b1, a2, b2 = paddle.s1(y, temp[1] // self.oup, axis=1)
            ...
            temp = y.shape
            a1, b1 = paddle.s1(y, temp[1] // self.oup, axis=1)
            a1 = a1 + self.init_a[0]
            b1 = b1 + self.init_b[0]
            out = x_out * a1 + x2 * b1
