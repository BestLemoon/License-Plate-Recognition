# author: avenuewl
# 5.26 图片输出列表splist_points数量控制；寻找上下边界；异常检测与记录；
# 6.20 异常处理；首尾端代码完善；分割点问题完善；
# 6.24 完成与pyqt5界面的数据传送和交互功能
# 6.25 完善添加首尾端的判断条件；分割点问题之条件完善；去除过近分割点的一处bug,使split_points其能够正常访问到尾部
...
def cut_char(path):
    ...
    cp2 = binary.copy()  # 绘制去除上下边界后的二值化图
    for j in range(0, h_start):
        for i in range(0, w):
            cp2[j, i] = 255
    for j in range(h_end, h):
    ...
    split_points = []
    i = 0
    while i < len(a) - 1:  # 找到水平方向上的符合分割条件的坐标信息
        if a[i] < 10:
            times = 1
            i = i + 1
            while a[i] < 10:
...
# cut_char('../reg/PaddleOCR/test_data/000005.jpg')